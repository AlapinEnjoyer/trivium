"""Verify add runtime recovery before its mutation is committed."""
...
def test_pending_add_restores_runtime_when_environment_sync_fails(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Restore the skill tree and lockfile when environment synchronization fails."""
    context = _make_context(tmp_path)
    installed_skill = context.install_path_for("alpha")
    installed_skill.mkdir(parents=True)
    installed_document = installed_skill / "SKILL.md"
    ...
    monkeypatch.setattr(add_module, "sync_active_environment", failed_sync)
